...
/**
 * The delivery service publishes its contract as OpenAPI, so the document version is the
 * same declaration as the package version and the two are never allowed to drift. The
 * article layer is not versioned here at all; it always ships from main.
 */
export function validateApiContractVersion({ packageVersion, openapiVersion }) {
  parseSemver(packageVersion, 'package.json version');
  parseSemver(openapiVersion, 'openapi info.version');
  assert.equal(
    openapiVersion,
    packageVersion,
    `OpenAPI info.version ${openapiVersion} must equal package.json version ${packageVersion}`,
  );
