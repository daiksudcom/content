...
/**
 * The abandoned article release control is spelled from parts so this repository keeps a
 * literal-free record of the flag provider it deliberately does not integrate with.
 */
const ABANDONED_CONTRACTS = new RegExp(
  [
    ['flag', 'ship'].join(''),
    'CONTENT_RELEASE_FLAG',
    'active-manifest',
    'smoke:release',
    'cache:purge',
    'isArticleOnly',
...
test('deploy keeps the running job, gates itself, and never touches feature flags', () => {
  ...
  assert.match(source, /group: content-production/);
  ...
  assert.doesNotMatch(source, ABANDONED_CONTRACTS, 'feature flags belong to the flags repository');
...
test('tagging and deploying stay in one workflow and survive a replay', () => {
  ...
  assert.equal(
    workflows.includes('delivery.yml'),
    false,
    'deploy.yml replaced the delivery workflow',
  );
});

test('the policy check runs the tests, the contract rule, and the title rule', () => {
  const source = readFileSync(join(workflowDirectory, 'ci.yml'), 'utf8');
  assert.match(source, /node --test \.github\/scripts\/\*\.test\.mjs/);
  assert.match(source, /policy\.mjs repository/, 'package and OpenAPI versions must agree');
  assert.match(source, /policy\.mjs pull-request/);
  assert.match(source, /BASE_SHA:/);
  assert.match(source, /PR_TITLE:/);
  assert.doesNotMatch(source, /RELEASE_ENABLED/);
  assert.doesNotMatch(source, /release\|rollback/, 'branch names never gate a release');

  const scripts = JSON.parse(readFileSync('package.json', 'utf8')).scripts;
  assert.equal(scripts['policy:test'], 'node --test .github/scripts/*.test.mjs');
  assert.equal(scripts['policy:check'], 'node .github/scripts/policy.mjs repository');
  assert.equal(scripts['test:policy'], undefined, 'the superseded script must be gone');
  assert.equal(scripts['validate:policy'], undefined, 'the superseded script must be gone');
  for (const name of ['policy:check', 'policy:test']) {
    assert.ok(scripts.validate.includes(name), `validate must run ${name}`);
  }
});

test('no feature flag or article release control remains in this repository', () => {
  const entries = readdirSync('.github');
  assert.equal(entries.includes('releases'), false, 'release descriptors are gone');
  for (const filename of readdirSync('.github/scripts')) {
    ...
    assert.doesNotMatch(source, ABANDONED_CONTRACTS, `${filename} still carries release control`);
  }
  for (const filename of workflows) {
    const source = readFileSync(join(workflowDirectory, filename), 'utf8');
    assert.doesNotMatch(source, ABANDONED_CONTRACTS, `${filename} still carries release control`);
  ...
  assert.doesNotMatch(
    packageSource,
    ABANDONED_CONTRACTS,
    'package.json still declares release-only scripts',
  );
});
...
test('repository reconciliation is additive and release notes keep the category order', () => {
  ...
  assert.match(settings, /context: CI success/);
  assert.match(settings, /context: Policy success/);
