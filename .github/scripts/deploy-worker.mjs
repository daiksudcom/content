...
/**
 * The delivery service is an API, so the smoke paths are the two endpoints that prove the
 * Worker resolved its content and can report the revision it is serving.
 */
const SMOKE_PATHS = ['/healthz', '/v1/version'];
...
async function smoke(origin, paths, label) {
  ...
  for (const path of paths) {
    ...
    url.searchParams.set('__content_smoke', sha.slice(0, 12));
    const status = await withRetries(`${label} smoke ${url}`, async () => {
      try {
        const response = await fetch(url, {
          headers: { 'cache-control': 'no-cache', 'user-agent': 'content-deploy-smoke/1' },
          redirect: 'follow',
...
const previewSmoke = await smoke(uploaded.preview_url, SMOKE_PATHS, 'preview');
...
let productionSmoke;
try {
  productionSmoke = await smoke(productionOrigin, SMOKE_PATHS, 'production');
} catch (error) {
