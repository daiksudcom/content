...
import { resolveTag, validateApiContractVersion, validateVersionBump } from './policy-lib.mjs';

const OPENAPI_PATH = 'openapi/content.openapi.json';
...
function packageVersion(revision) {
  const { version } = readJson(revision, 'package.json');
  assert(version, 'package.json must declare a version');
  return version;
}

function openapiVersion(revision) {
  const document = readJson(revision, OPENAPI_PATH);
  assert(document.info?.version, `${OPENAPI_PATH} must declare info.version`);
  return document.info.version;
}

/**
 * The delivery service layer is what carries SemVer, and its published contract is the
 * OpenAPI document, so the two versions are checked together on every revision. Articles
 * under blog/** are outside this check because they always ship from main.
 */
function validateRepository(revision = null) {
  const version = validateApiContractVersion({
    packageVersion: packageVersion(revision),
    openapiVersion: openapiVersion(revision),
  });
  process.stdout.write(`package.json and ${OPENAPI_PATH} both declare ${version}.\n`);
  return version;
}
...
function resolveRevision(sha) {
  ...
  validateApiContractVersion({
    packageVersion: packageVersion(sha),
    openapiVersion: openapiVersion(sha),
  });
