...
Contentは性質の異なる二つのlayerを同じWorkerから配信します。versioningの扱いも異なります。

| layer | 対象 | versioning |
| --- | --- | --- |
| delivery service layer | API handler、OpenAPI契約、Worker実装 | SemVerを維持する。`package.json#version`とOpenAPI `info.version`は常に一致する |
| content layer | `blog/**`の記事MDXと記事media | 常に`main`を配信する。release制御を一切持たない |

Contentはfeature flagを保持しません。flagの状態はflags repositoryが正本として管理し、Contentは他のconsumerと同じようにそれを参照して機能のオンオフを決めます。

したがってContentで起こるproduction操作はDeployだけです。

- **Deploy**: PRがmergeされるたびに必ず実施する。`main`のrevisionをWorkerへ反映する。**flagで囲まれた機能は公開しない。** 不具合修正の反映は許容する。
- **Product Release**: flags repositoryでflagをONにすること。**Contentを再deployせずに機能が公開される。**

記事の公開はDeployそのものです。記事はflagでもmanifestでも隔離されません。
...
記事だけのPRは`docs(content): ...`です。**他のrepositoryにおける`docs:`と完全に同じ扱いで**、SemVer coreを変えず、Deployされ、build metadata tagを受け取ります。記事にContent固有の例外規則はありません。
...
## OpenAPI契約とversionの一致

delivery service layerの公開契約は`openapi/content.openapi.json`です。`policy.mjs repository`が`package.json#version`と`info.version`の一致を検証し、CIの`Policy success`と`pnpm validate`の両方から実行します。両者が食い違ったrevisionはmergeもDeployもできません。
...
外部要因と判断できる失敗だけを**最大3回**リトライします。リトライはtagを打ち直しません。
...
| `PRODUCTION_ORIGIN` | smoke対象の`https://content.daiksud.me` |
...
**Deploy tokenにfeature flagを変更する権限を与えてはいけません。** flagを変更できるのはflags repositoryだけです。GitHub workflowのdefault permissionはreadで、すべてのthird-party Actionはfull commit SHAで固定します。
